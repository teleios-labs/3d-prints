"""Capture golden scalar values for case geometry tests.

Run after an intentional geometry change to refresh the golden
constants in ``test_case_geometry.py``:

    uv run python esp-screen-case/tests/capture_case_golden.py
...
from esp_screen_case.case import build  # noqa: E402

TEST_FILE = Path(__file__).parent / "test_case_geometry.py"


def capture() -> dict[str, str]:
    """Build the case part and return formatted golden values."""
    with tempfile.TemporaryDirectory() as tmp:
        stl_path = Path(tmp) / "case.stl"
...
def main() -> None:
    ...
    parser.add_argument(
        "--write",
        action="store_true",
        help="rewrite GOLDEN_* assignments in test_case_geometry.py",
    )
