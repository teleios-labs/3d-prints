...
in ``test_tolerance_test_geometry.py``:
...
TEST_FILE = Path(__file__).parent / "test_tolerance_test_geometry.py"


def capture() -> dict[str, str]:
    """Build the tolerance part and return formatted golden values."""
    ...
    def fmt_bbox(extents) -> str:
        return "(%.4f, %.4f, %.4f)" % tuple(extents)

    return {
        "GOLDEN_TOTAL_VOLUME": "%.4f" % mesh.volume,
        "GOLDEN_TOTAL_BBOX": fmt_bbox(mesh.bounding_box.extents),
        "GOLDEN_A_VOLUME": "%.4f" % piece_a.volume,
        "GOLDEN_A_BBOX": fmt_bbox(piece_a.bounding_box.extents),
        "GOLDEN_A_CENTROID_Y": "%.4f" % piece_a.centroid[1],
        "GOLDEN_B_VOLUME": "%.4f" % piece_b.volume,
        "GOLDEN_B_BBOX": fmt_bbox(piece_b.bounding_box.extents),
        "GOLDEN_B_CENTROID_Y": "%.4f" % piece_b.centroid[1],
    }
...
def main() -> None:
    ...
    parser.add_argument(
        "--write",
        action="store_true",
        help="rewrite GOLDEN_* assignments in test_tolerance_test_geometry.py",
    )
